...
if [[ "$DB_URL_EFFECTIVE" == *"@db/"* ]]; then
  ...
  if [[ -n "$DB_IP" ]]; then
    ...
    echo "INFO: DB host 'db' replaced with container IP ${DB_IP} for local run"
  else
...
export WEB_API_PORT="$PORT"

echo "INFO: Starting web_api on port ${WEB_API_PORT}"
